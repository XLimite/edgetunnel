...
async function getAllConfigVless(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const vlessTls = `vless://freeoo\u0040${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=${pathFixed}#freeoo (${data.country})`;
            const vlessNtls = `vless://freeoo\u0040${hostName}:80?path=${pathFixed}&security=none&encryption=none&host=${hostName}&fp=randomized&type=ws&sni=${hostName}#freeoo (${data.country})`;
            const vlessTlsFixed = vlessTls.replace(/ /g, '+');
            const vlessNtlsFixed = vlessNtls.replace(/ /g, '+');
            allConfigs += 
`    <div class="blog-slider__item swiper-slide">
      <div class="blog-slider__img">
        <img src="https://picsum.photos/1280/720?random=${data.city}" alt="">
      </div>
      <div class="blog-slider__content">
        <span class="blog-slider__code">${path}</span>
        <div class="blog-slider__title">${data.isp}</div>
        <div class="blog-slider__text">
		 <p><strong>ASN: </strong> ${data.as}</p>
		 <p><strong>City: </strong> ${data.city}</p>
		 <p><strong>Country: </strong>${data.country}</p>
		</div>
		<button class="blog-slider__button" onclick='copyToClipboard("${vlessTlsFixed}")'><i class="fa fa-clipboard"></i>Copy VLESS TLS</button>
      </div>
    </div>
`;
}
...
<head>
    ...
    <link rel="stylesheet" href="https://kuyshare.pages.dev/style.css">
</head>
<body>
...
<script  src="https://kuyshare.pages.dev/script.js"></script>
</body>
</html>`;
